...
namespace ZhengJuyin.UI
{
    public partial class ZGraph : UserControl
    {
        ...
        {
            #region **绘图参数初始化**
            var width = pictureBoxGraph.Width;
            var height = pictureBoxGraph.Height;
            var Grap = e.Graphics;
            #endregion

            using (var pe = new Pen(Color.White, 1f))
            {
                ...
                else if (_isAutoModeXY)
                {
                    ...
                    //遍历每条数据集合
                    for (var i = 0; i < _listX.Count; i++)
                    {
                        //遍历数据集合中的每个点
                        for (var j = 0; j < _listX[i].Count; j++)
                        {
                            ...
                            else if (_listX[i][j] > _fXEnd)
                            {
                                ...
                                //_fXEndGO = ((int)(_listX[i][j] / _fXQuanEndGO) + 1) * _fXQuanEndGO;
                                //_fXEnd = _listX[i][j];
                                //_fXBegin = _fXEnd - 100;
                            }
                ...
                for (var i = 0; i < _listX.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        {
                            //绘制方形点
                            foreach (var points in _listDrawPoints)
                            {
        ...
        private void pictureBoxBottom_Paint(object sender, PaintEventArgs e)
        {
            ...
            #region **参数初始化**
            var width = pictureBoxBottom.Width;
            var height = pictureBoxBottom.Height;
            ...
            {
                using (var pe = new Pen(_coordinateLineColor, 2f))
                {
                    using (var brushString = new SolidBrush(_coordinateStringColor))
                    {
                        using (var format = new StringFormat())
                        {
                            using (var fo = new Font("宋体", 9))
                            {
                                #region **画背景色**
                                ...
                                if (_fXQuanBeginGO <= _fXQuanEndGO)
                                {
                                    ...
                                    pxwidth = (width - 100)/(_fXBegin - _fXEnd)*(_fXBeginGO - _fXEndGO); //所要画坐标的像素范围
                                    ...
                                    if (pxLine <= 250) //若间距够画第二层坐标并显示坐标值的话，就不需要话第一层坐标
                                    {
                                        ...
                                        for (var i = 0; i < currentI; i++)
                                        {
                                            if (currentDraw > 50 && currentDraw < width50)
                                            {
                                                ...
                                                Grap.DrawString(showTextT.ToString(), fo, brushString, currentDraw, 6,
                                                    format); //画坐标值
                                            }
                                ...
                                else
                                {
                                    ...
                                    pxLine = pxwidth/linesNum; //每段坐标线间隔
                                    _fXLinesShowFirst = pxLine; //标记【X轴第一层网格线间隔】
                                    pxGO = 50 - (_fXBegin - _fXBeginGO)/(_fXEnd - _fXBegin)*(width - 100);
                                        //所要画坐标的起点像素位置
                                    _fXpxGO = pxGO - 51; //标记【所要画X轴坐标的起点像素位置】
                                    #endregion
                                ...
                                Grap.DrawString(string.Format("{0}", Math.Round(_fXBegin, _iAccuracy)), fo, brushString,
                                    51, 10, format); //X轴起始坐标值
                                format.Alignment = StringAlignment.Near; //文字水平居左对齐
                                Grap.DrawLine(pe, width - 50, 0, width - 50, 10); //X轴结束坐标线条
                                Grap.DrawString(string.Format("{0}", Math.Round(_fXEnd, _iAccuracy)), fo, brushString,
                                    width - 60, 10, format); //X轴结束坐标值
                                #endregion
        ...
        {
            #region **参数初始化**
            var width = pictureBoxLeft.Width;
            var height = pictureBoxLeft.Height;
            float linesQuan; //获得两权的大值
            float linesNum; //可以分成的线段
            float pxLine; //每段坐标线间隔
            float pxwidth; //所要画坐标的像素范围
            ...
            using (var brush = new SolidBrush(_backColorL))
            {
                using (var pe = new Pen(_coordinateLineColor, 2f))
                {
                    using (var brushString = new SolidBrush(_coordinateStringColor))
                        ...
                        using (var format = new StringFormat())
                        {
                            using (var fo = new Font("宋体", 9))
                            {
                                ...
                                if (_fYQuanBeginGO <= _fYQuanEndGO)
                                {
                                    ...
                                    pxwidth = (height - 30)/(_fYBegin - _fYEnd)*(_fYBeginGO - _fYEndGO); //所要画坐标的像素范围
                                    pxLine = pxwidth/linesNum; //每段坐标线间隔
                                    ...
                                    if (pxLine <= 150) //若间距够画第二层坐标并显示坐标值的话，就不需要话第一层坐标
                                    {
                                        ...
                                        for (var i = 0; i < currentI; i++)
                                        {
                                            if (currentDraw > 30 && currentDraw < height)
                                            {
                                                ...
                                                Grap.DrawString(showTextT.ToString(), fo, brushString, 44, currentDraw,
                                                    format); //画坐标值
                                            }
                                            showTextT -= (decimal) linesQuan; //更新要画的坐标值
                                            currentDraw += pxLine; //更新坐标线和坐标值的位置
                                ...
                                Grap.DrawString(string.Format("{0}", Math.Round(_fYBegin, _iAccuracy)), fo, brushString,
                                    26, height - 2, format); //Y轴起始坐标值
                                Grap.DrawLine(pe, 26, 31, 50, 31); //Y轴结束坐标线条
                                Grap.DrawString(string.Format("{0}", Math.Round(_fYEnd, _iAccuracy)), fo, brushString,
                                    26, 29, format); //Y轴结束坐标值
                                #endregion
                            }
                        }
                    }
        ...
        {
            #region **参数初始化**
            var width = pictureBoxTop.Width;
            var height = pictureBoxTop.Height;
            var Grap = e.Graphics;
            #endregion
            ...
            using (var brush = new SolidBrush(_backColorL))
            {
                using (var format = new StringFormat())
                {
                    using (var fo = new Font("宋体", 9))
                    {
                        using (var foTitle = new Font("宋体", _titleSize, FontStyle.Bold))
                        {
                            #region **画背景色**
                            Grap.FillRectangle(brush, e.ClipRectangle);
                            #endregion
                            ...
                            Grap.DrawString(_SyStitle, foTitle, brush, pictureBoxTop.Width*_titlePosition + 1,
                                height + 1, format);
                            Grap.DrawString(_SyStitle, foTitle, brush, pictureBoxTop.Width*_titlePosition - 1,
                                height - 1, format);
        ...
        {
            #region **参数初始化**
            var width = pictureBoxRight.Width;
            var height = pictureBoxRight.Height;
            var Grap = e.Graphics;
            #endregion
